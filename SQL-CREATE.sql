...
CREATE TABLE Teacher(
    ...
    teacher_patronym VARCHAR(50) NULL,
...
CREATE TABLE Timetable(
    ...
    timetable_class VARCHAR(10) NOT NULL CHECK ( timetable_class ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$')
);
...
CREATE TABLE Journal(
    ...
    journal_teacher INT NOT NULL REFERENCES Teacher(teacher_id),
...
CREATE TABLE Days(
    ...
    day_timetable INT NOT NULL REFERENCES Timetable(timetable_id)
);
CREATE TABLE Class(
    class_name VARCHAR(10) PRIMARY KEY CHECK ( class_name ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$'),
    class_journal_id INT NOT NULL REFERENCES Journal(journal_id),
    class_mainTeacher INT NOT NULL UNIQUE REFERENCES Teacher(teacher_id)
);
ALTER TABLE Timetable
ADD FOREIGN KEY (timetable_class) REFERENCES Class(class_name);
CREATE TABLE Lessons(
    lesson_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    lesson_name VARCHAR(50) UNIQUE,
    lesson_class VARCHAR(10) NOT NULL REFERENCES Class(class_name) CHECK ( lesson_class ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$'),
...
CREATE TABLE Homework(
    ...
    homework_teacher INT NOT NULL REFERENCES Teacher(teacher_id),
    homework_lesson INT NOT NULL REFERENCES Lessons(lesson_id),
    homework_duedate DATE NOT NULL,
    homework_created_at DATE DEFAULT CURRENT_DATE NOT NULL,
    homework_desc TEXT NOT NULL,
    homework_class VARCHAR(10) NOT NULL REFERENCES Class(class_name) CHECK ( homework_class ~ '^(?:[1-9]|1[0-2])-([А-ЩЬЮЯҐЄІЇ]|[а-щьюяґєії])$')
);
CREATE TABLE Students(
    ...
    student_class VARCHAR(10) NOT NULL REFERENCES Class(class_name)
);
...
CREATE TABLE StudentParent (
    student_id_ref INT REFERENCES Students(student_id),
    parent_id_ref INT REFERENCES Parents(parent_id),
...
CREATE TABLE StudentData(
    ...
    lesson INT NOT NULL REFERENCES Lessons(lesson_id),
